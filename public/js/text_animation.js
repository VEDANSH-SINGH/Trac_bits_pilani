...
if (vh > 600) {
  adjustY2 =
    window.innerHeight / (2 * textScale) + 69 + 24 / (0.9 * whr ** 1.1);
} else {
...
function Init() {
  ...
  for (let m = 0, m2 = textCoordinates2.height; m < m2; m++) {
    for (let n = 0, n2 = textCoordinates2.width; n < n2; n++) {
      if (
        textCoordinates2.data[m * 4 * textCoordinates2.width + n * 4 + 3] > 128
      ) {
        let positionX2 = n + adjustX2;
        let positionY2 = m + adjustY2;
        particleArray3.push(
          new Particle(positionX2 * textScale2, positionY2 * textScale2)
        );
      }
    }
  }
